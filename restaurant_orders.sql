...
-- 2. Which promo codes generate the highest number of successful deliveries?
...
-- 4. Are orders increasing or decreasing over months?
SELECT 
...
-- 5. Inactive Customers (No Orders Recently)
...
-- 6. How are orders trending over months?
SELECT 
    DATE_FORMAT(Placed_at, '%Y-%m') AS month,
    COUNT(Order_id) AS total_orders
FROM orders
...
-- 7. Which customers placed more than one order?
SELECT 
    Customer_code,
    COUNT(Order_id) AS total_orders
FROM orders
